/*$(".foot .changepcbtn .pc").click(function (e) { 
    e.preventDefault();
	console.log('click');
    setCookie001("changepc", 1)//点击电脑端则为1
    location.reload();
});

function getCookie001(c_name) {
...
function setCookie001(name, value) {
	document.cookie = name + "=" + encodeURIComponent(value) ;
	return true;
};

var changepc=getCookie001('changepc');//获取cook
if(changepc){
   $("meta[name='viewport']").attr("content","");//电脑端
   $(".Phone_Css").remove();
}


$(".foot .changepcbtn span.pc").click(function (e) { 
        e.preventDefault();
        setCookie("changepc", 1)//点击电脑端则为1
		location.reload();
...
// }
    ...
    // }else{
        if (k.replace(/\s+/g,'') == null || k.replace(/\s+/g,'') === "") {
            alert("请输入搜索内容");
            return false;
        } else {
           /* url="/search/s?qt="+encodeURIComponent(k)+"&siteCode=5100000062&tab=qs&toolsStatus=1";*/
            url='/search/s?tab=all&qt='+encodeURIComponent(k)+'&siteCode=5100000062&toolsStatus=1'
...
// 			url = "/search/";
// 		} else {
// url="/search/s?qt="+k+"&siteCode=5100000062&tab=qs&toolsStatus=1"
// 			/*url = "/so4/s?uc=1&column=" + encodeURI(encodeURI("全站")) + "&searchWord=" + encodeURI(encodeURI(k)) + "&columnId=0";*/
// 		}
// 	}
// 	window.open(url);
...
// function SearchUsxys() {
// 	var k = document.getElementById("keywordxys").value;
// 	var url = "/search/";
// 	if(k=="健康证明在线申报"){
// 		url="http://jkcx.sc.gov.cn/jkxxtb/epidemic/personnelRegistrationCondition/toDeclareMainView"
// 	}else{
...
function SearchUsxys() {
    ...
    // }else{
        if (k.replace(/\s+/g,'') == null || k.replace(/\s+/g,'') === "") {
           ...
           /* url="/search/s?qt="+encodeURIComponent(k)+"&siteCode=5100000062&tab=qs&toolsStatus=1";*/
            url='/search/s?tab=all&qt='+k+'&siteCode=5100000062&toolsStatus=1'
            /*url = "/so4/s?uc=1&column=" + encodeURI(encodeURI("全站")) + "&searchWord=" + encodeURI(encodeURI(k)) + "&columnId=0";*/
        }
    // }
    window.open(url);
